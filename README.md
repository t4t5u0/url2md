...
- X / Twitter の投稿は、定型部分と本文中の `t.co` 短縮 URL を外して `投稿者: 本文` にします

```
変換前  XユーザーのAUTOMATON（オートマトン）さん: 「【ニュース】…無法漁師生活 https://t.co/lZsxQEXuZE https://t.co/qaC3s08D6O」 / X
変換後  AUTOMATON（オートマトン）: 【ニュース】…無法漁師生活
```

`(2026) ` のような 4 桁の数字は年の可能性があるためタイトルの一部として残します。
サイト名のサフィックス（` - YouTube` など）は X 以外では残します。
